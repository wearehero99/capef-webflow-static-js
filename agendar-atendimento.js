  ...
  async function getTimes({ day, year, month, atendimentoType }) {
    ...
    try {
      ...
      if (data.status === 204) {
        //Sem horarios
        console.log("Sem horarios", [])
        showFormFailMessage("Sem horarios")
  ...
  async function getTimesOfToday() {
    ...
    getTimes({
      day: currentDay, year: currentYear, month: currentMonth, atendimentoType: planInputValue
    })
  }
  ...
  async function isAttendAlreadyExist({ typeAtt, cpf }) {
    ...
    if (result.status === 204) {
      ...
      console.log("Não existe agendamento gravados que atendem aos parâmetros passados.")
      return true;
    } else {
      const data = (await response).json()
      return false
  ...
  async function getPlans() {
    ...
    const timeInput = $("#plan-input");

    $.each(result, function (index, value) {
      timeInput.append("<option value='" + value.id + "'>" + value.descricao + "</option>");
    });

    timeInput.val(result[0]);
  }
  ...
  async function createRegistration() {
    ...
    if (cpfInputValue && timeInputValue && planInputValue && emailInputValue && phoneValue) {
      ...
      console.log("horario ===> ", timeInputValue)
